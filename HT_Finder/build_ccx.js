...
output.on("close", () => process.stdout.write(`${archive.pointer()} bytes written\n`));
archive.pipe(output);
const manifest = JSON.parse(fs.readFileSync(path.join(stageDir, "manifest.json"), "utf8"));
archive.append(JSON.stringify(manifest, null, 2), { name: "manifest.json" });
for (const file of files(stageDir).filter(file => file !== "manifest.json")) archive.file(path.join(stageDir, file), { name: file.split(path.sep).join("/") });
archive.finalize();
